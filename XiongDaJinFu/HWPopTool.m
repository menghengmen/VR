...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.popBackgroudColor = [UIColor blackColor];
    };
    return self;
}
...
- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {
    ...
    //自定义btn
    UIButton  * meiBtn = [UIButton new];
    [meiBtn layoutButtonWithEdgeInsetsStyle:MKButtonEdgeInsetsStyleTop imageTitleSpace:0];
     [meiBtn setBackgroundImage:[UIImage imageNamed:@"mei"] forState:UIControlStateNormal];
    [meiBtn setTitle:@"美国" forState:UIControlStateNormal];
    meiBtn.layer.cornerRadius = 40;
    meiBtn.clipsToBounds = YES;
    [meiBtn addTarget:self action:@selector(close:) forControlEvents:UIControlEventTouchUpInside];
    [presentView addSubview:meiBtn];
    [meiBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(presentView.mas_centerX);
        make.bottom.equalTo(presentView.mas_centerY).offset(-60);
    ...
    [enBtn setTitle:@"英国" forState:UIControlStateNormal];

    [enBtn setBackgroundImage:[UIImage imageNamed:@"ying"] forState:UIControlStateNormal];
    [enBtn addTarget:self action:@selector(close:) forControlEvents:UIControlEventTouchUpInside];
    [enBtn layoutButtonWithEdgeInsetsStyle:MKButtonEdgeInsetsStyleTop imageTitleSpace:0];

    enBtn.layer.cornerRadius = 40;
    enBtn.clipsToBounds = YES;
    [presentView addSubview:enBtn];
    [enBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(presentView.mas_centerX);
        make.top.equalTo(presentView.mas_centerY).offset(60);
        make.width.equalTo(@80);
        make.height.equalTo(@80);
...
- (void)close:(UIButton*)senser {
    [self hideAnimated:YES withCompletionBlock:nil];
    self.selectedCityBlock(senser.titleLabel.text);
}
- (void)closeWithBlcok:(void(^)())complete {
    [self hideAnimated:YES withCompletionBlock:complete];
}
-(void)cityChoose:(chooseSuccess)cityNameBlock{

    self.selectedCityBlock = cityNameBlock;
