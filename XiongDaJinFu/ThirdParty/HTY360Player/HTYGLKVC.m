...
#define ROLL_CORRECTION ES_PI*3/2
...
- (void)viewDidLoad {
  ...
  _preferredConversion = kColorConversion709;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pinchGesture:) name:@"pinchGesture" object:nil];
}

-(UIInterfaceOrientationMask) supportedInterfaceOrientations {
    if (_videoPlayerController.isGuideVideo == YES) {
...
- (void)startDeviceMotion {
  ...
  _isUsingMotion = YES;
    ...
    if([self isPortrait])
    {
        roll_correction = ES_PI/0.5;
        roll_correction2 = -ES_PI*0.5;
...
//      //float cPitch = attitude.pitch/2; // Depth en landscape -> pas besoin de prendre l'opposé
...
//      }
      ...
      if (YES) {
        ...
        //modelViewMatrix = GLKMatrix4RotateZ(modelViewMatrix, roll_correction2);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  distX *= -0.005;
  distY *= -0.005;
    NSDictionary * aDic = @{@"distY":@(distY),
                            @"distX":@(distX)};
    [[NSNotificationCenter defaultCenter] postNotificationName:@"touchMove" object:nil userInfo:aDic];
}
- (void)touchMove:(NSNotification*)notify {
    NSDictionary * aDic = notify.userInfo;
    float distY = [[aDic objectForKey:@"distY"] floatValue];
    float distX = [[aDic objectForKey:@"distX"] floatValue];
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer *)recognizer {
  _overture /= recognizer.scale;
    NSDictionary * aDic = @{@"overture":@(_overture)};
  [[NSNotificationCenter defaultCenter] postNotificationName:@"pinchGesture" object:nil userInfo:aDic];
...
- (void)pinchGesture:(NSNotification*)notify {
    NSDictionary * aDic = notify.userInfo;
    _overture = [[aDic objectForKey:@"overture"] floatValue];
}
