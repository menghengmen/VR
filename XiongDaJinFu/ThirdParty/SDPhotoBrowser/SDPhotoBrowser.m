...
- (void)setupToolbars
{
    ...
    //图片描述
    UILabel  * describtionLabel  = [UILabel new];
//    if (self.imageCount > 1) {
//        describtionLabel.text = self.titleArray[_imageCount-1];
//    }

    describtionLabel.textColor = [UIColor whiteColor];
    
    describtionLabel.bounds =  CGRectMake(0, 0, 200,100);
    
    _desLabel = describtionLabel;
...
- (void)layoutSubviews
{
    ...
    _desLabel.frame = CGRectMake(SCREENWIDTH/2, self.bounds.size.height - 70, 100, 25);
...
- (void)showFirstImage
{
    ...
    [self addSubview:tempView];
    UIView *view = _scrollView.subviews[self.currentImageIndex];
    CGRect targetTemp = view.bounds;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        ...
        _desLabel.text = self.titleArray[index];
