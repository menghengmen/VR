...
//    [glkViewController.view setFrame:CGRectMake(0, 0, width, height)];
    ...
    [glkViewController.view mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(glkViewController.view.superview);
        make.right.equalTo(glkViewController.view.superview).offset(55);
        make.top.equalTo(glkViewController.view.superview).offset(-64);
        make.height.equalTo(@(viewController.view.bounds.size.height + 64));
    }];
    glkViewController.view.backgroundColor = LRRGBColor(222, 222, 222);
    
    
    parentView.frame = CGRectMake(0, 0, SCREENWIDTH, 0);
