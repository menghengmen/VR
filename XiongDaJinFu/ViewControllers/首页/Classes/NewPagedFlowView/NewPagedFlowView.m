...
- (void)startTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:10000 target:self selector:@selector(autoNextPage) userInfo:nil repeats:YES];
        ...
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width) {
                    ...
                    cell.mainImageView.frame = CGRectMake(0, 0, cell.frame.size.width, cell.frame.size.height);
                } else {
        ...
        case NewPagedFlowViewOrientationVertical:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.height) {
                    ...
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(inset, inset, inset, inset));cell.mainImageView.frame = CGRectMake(0, 0, cell.frame.size.width, cell.frame.size.height);
                } else {
                    cell.coverView.alpha = _minimumPageAlpha;
